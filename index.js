...
export default async function handler(req, res) {
  // ⚡ CORS Headers (බොට් එකට සහ බ්‍රවුසර් එකට ලේසි වෙන්න)
  ...
  try {
    if (!action) return res.status(200).json({ status: true, message: "Cineru API is Live 🚀" });
    ...
    if (action === "movie") {
      ...
      $("a.wp-block-button__link").each((i, el) => {
        ...
        if (btnLink && (btnLink.includes("dl.cineru.lk") || btnLink.includes("pixeldrain"))) {
          dl_links.push({ quality: btnTitle, direct_link: btnLink });
        }
      });

      return res.json({ status: true, data: { title: $("h1").first().text().trim(), download_links: dl_links } });
    }
  } catch (err) {
